...
class WBRefreshControl: UIControl {
    ...
    fileprivate let maxDistance:CGFloat      =   30; //下拉多少可以松开
    ...
    //下拉刷新旋转的样式
    internal var refreshActivity:UIActivityIndicatorView!; // 菊花视图
    ...
    init(scrollView:UIScrollView, activityIndicatorView activity:UIView?,refreshBlock:@escaping ()->(),loadmoreBlock:@escaping ()->()){
        ...
        self.remindLabel.font = UIFont.systemFont(ofSize: 11)
        self.remindLabel.textAlignment = NSTextAlignment.center
        self.remindLabel.text = "正在加载..."
//        self.addSubview(self.remindLabel)
        
        //图标背景颜色
        shapeTintColor = UIColor.lightGray
        layerAdd();
    ...
    fileprivate func refreshActivityShow()->Void{
        ...
        self.remindLabel.frame =  CGRect(x: 0, y: 0, width: 414, height: 20)
        self.remindLabel.center = CGPoint(x: floor(self.frame.size.width / 2), y: 20)
    ...
    fileprivate func refreshActivityHide()->Void{
        ...
        self.remindLabel.frame =  CGRect(x: 0, y: 0, width: 414, height: 0)
            ...
            if (offset != 0) {
                ...
                if (offset < 0) {
                    if (offset >= -showViewHeight) {
                        ...
                        self.remindLabel.frame =  CGRect(x: 0, y: 0, width: 414, height: 20)
                        self.remindLabel.center = CGPoint(x: floor(self.frame.size.width / 2), y: totalViewHeight - showViewHeight/2 + 20)
                    }
                }
                ignoreInset = false;
                ignoreOffset = false;
